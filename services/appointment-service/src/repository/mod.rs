...
pub async fn create_slots(
    ...
    for (start, end) in slots {
        let slot = sqlx::query_as::<_, TimeSlot>(
            "INSERT INTO time_slots (provider_id, start_time, end_time) VALUES ($1, $2, $3) RETURNING *",
        )
        .bind(provider_id)
        .bind(start)
        .bind(end)
...
pub async fn find_available_slots(
    ...
    let slots = if let Some(date_str) = date {
        sqlx::query_as::<_, TimeSlot>(
            "SELECT * FROM time_slots WHERE provider_id = $1 AND status = 'available' AND start_time::date = $2::date ORDER BY start_time",
        )
        .bind(provider_id)
        .bind(date_str)
        .fetch_all(pool)
        ...
        sqlx::query_as::<_, TimeSlot>(
            "SELECT * FROM time_slots WHERE provider_id = $1 AND status = 'available' AND start_time > NOW() ORDER BY start_time",
        )
        .bind(provider_id)
        .fetch_all(pool)
        .await
...
pub async fn create_appointment(
    ...
    sqlx::query_as::<_, Appointment>(
        "INSERT INTO appointments (slot_id, customer_id, provider_id, status) VALUES ($1, $2, $3, 'confirmed') RETURNING *",
    )
    .bind(slot_id)
    .bind(customer_id)
    .bind(provider_id)
...
pub async fn find_slot_by_id(pool: &PgPool, slot_id: Uuid) -> Result<Option<TimeSlot>, AppError> {
    sqlx::query_as::<_, TimeSlot>("SELECT * FROM time_slots WHERE id = $1")
        .bind(slot_id)
        .fetch_optional(pool)
        .await
        .map_err(|e| AppError::Internal(e.to_string()))
...
pub async fn find_appointments_by_customer(
    ...
    sqlx::query_as::<_, Appointment>(
        "SELECT * FROM appointments WHERE customer_id = $1 ORDER BY created_at DESC",
    )
    .bind(customer_id)
    .fetch_all(pool)
    .await
...
pub async fn find_appointments_by_provider(
    ...
    sqlx::query_as::<_, Appointment>(
        "SELECT * FROM appointments WHERE provider_id = $1 ORDER BY created_at DESC",
    )
    .bind(provider_id)
    .fetch_all(pool)
    .await
